...
function App() {
  ...
  const [showHistoryInfoArray, setShowHistoryInfoArray] = useState([]); // State to manage visibility of each contract's update history
  ...
  useEffect(() => {
    // Initialize showHistoryInfoArray with false values for each contract
    setShowHistoryInfoArray(Array(collectionData.length).fill(false));
  }, [collectionData]);


  // Function to toggle the visibility of a contract's update history based on its index
  const toggleHistoryInfo = (index) => {
    setShowHistoryInfoArray(prevState => {
      const newState = prevState.map((value, idx) => {
        if (idx === index) {
          // Toggle the clicked contract's update history
          return !value;
        } else {
          // Close the update history for other contracts within the same index
          return false;
        }
      });
      return newState;
                ...
                <div className=''>
                  ...
                  <div className="block max-w p-6 bg-white border border-gray-200 rounded-lg shadow  dark:bg-gray-800 dark:border-gray-700 ">
                    ...
                    <div className=' '>
                      ...
                      <div>
                        <button
                          onClick={() => toggleHistoryInfo(index)}
                          className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                        >
                          {showHistoryInfoArray[index] ? 'Hide Update History' : 'Show Update History'}
